 ...
 */
@Mod.EventBusSubscriber(modid = CreateNestNetwork.MOD_ID, bus = Mod.EventBusSubscriber.Bus.MOD,
    value = Dist.CLIENT)
public final class CnnModelEvents {
    ...
    public static void registerLiveryModels(ModelEvent.RegisterAdditional event) {
        event.register(CreateNestNetwork.asResource("block/transit_frogport_base"));
        for (String dye : DYES)
            for (String state : new String[] { "closed", "open" })
                for (int facing : FACINGS.values())
                    event.register(CreateNestNetwork.asResource(
    ...
    private static ResourceLocation liveryFor(ModelResourceLocation id) {
        ...
        if ("package_frogport".equals(path))
            return CreateNestNetwork.asResource("block/transit_frogport_base");
        ...
        if (facing == null)
            return null;
        return CreateNestNetwork.asResource("block/transit_" + dye + "_postbox_" + state + "_y" + facing);
    }
